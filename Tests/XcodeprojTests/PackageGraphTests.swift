...
class PackageGraphTests: XCTestCase {
    func testBasics() throws {
      let fs = InMemoryFileSystem(emptyFiles:
        ...
        let project = try xcodeProject(xcodeprojPath: AbsolutePath.root.appending(component: "xcodeproj"), sdkRoot: .macosx, version: "10.10", graph: g, extraDirs: [], options: options, fileSystem: fs)
    ...
    func testAggregateTarget() throws {
        ...
        let project = try xcodeProject(xcodeprojPath: AbsolutePath("/Foo/build").appending(component: "xcodeproj"), sdkRoot: .macosx, version: "10.10", graph: g, extraDirs: [], options: XcodeprojOptions(), fileSystem: fs)
        XcodeProjectTester(project) { result in
            result.check(target: "Foo") { targetResult in
                targetResult.check(productType: .framework)
                targetResult.check(dependencies: [])
    ...
    func testModulemap() throws {
      ...
      let project = try xcodeProject(xcodeprojPath: AbsolutePath("/Bar/build").appending(component: "xcodeproj"), sdkRoot: .macosx, version: "10.10", graph: g, extraDirs: [], options: XcodeprojOptions(), fileSystem: fs)
    ...
    func testModuleLinkage() throws {
        ...
        let project = try xcodeProject(xcodeprojPath: AbsolutePath.root.appending(component: "xcodeproj"), sdkRoot: .macosx, version: "10.10", graph: g, extraDirs: [], options: XcodeprojOptions(), fileSystem: fs)
